 ...
 * Description: A professional, production-ready WordPress plugin delivering essential, high-performance UI widgets and layout modules.
 * Version: 1.0.0
 ...
 * @return bool
 */
function pressiq_is_blocks_module_active() {
    $options = get_option( 'pressiq_options', array() );
 ...
 * @return bool
 */
function pressiq_has_elementor() {
...
function pressiq_load_plugin() {
    ...
    if ( version_compare( PHP_VERSION, PRESSIQ_MINIMUM_PHP_VERSION, '<' ) ) {
        add_action( 'admin_notices', 'pressiq_admin_notice_minimum_php_version' );
        return;
    }

    $has_elementor   = pressiq_has_elementor();
    $blocks_active   = pressiq_is_blocks_module_active();

    // If neither Elementor nor blocks module is available, show notice
    if ( ! $has_elementor && ! $blocks_active ) {
        add_action( 'admin_notices', 'pressiq_admin_notice_missing_elementor' );
...
add_action( 'plugins_loaded', 'pressiq_load_plugin' );

/**
 * Admin notice: Elementor not installed (shown only when blocks module is also inactive)
 */
function pressiq_admin_notice_missing_elementor() {
 ...
 */
function pressiq_admin_notice_minimum_elementor_version() {
    if ( isset( $_GET['activate'] ) ) {
        unset( $_GET['activate'] );
    }
 ...
 */
function pressiq_admin_notice_minimum_php_version() {
    if ( isset( $_GET['activate'] ) ) {
        unset( $_GET['activate'] );
    }
...
function pressiq_activate() {
    ...
    if ( ! get_option( 'pressiq_options' ) ) {
        add_option( 'pressiq_options', $default_options );
    }

    // Flush rewrite rules
    flush_rewrite_rules();
}
register_activation_hook( __FILE__, 'pressiq_activate' );

/**
 * Plugin deactivation hook
 */
function pressiq_deactivate() {
    // Flush rewrite rules
    flush_rewrite_rules();
}
register_deactivation_hook( __FILE__, 'pressiq_deactivate' );
